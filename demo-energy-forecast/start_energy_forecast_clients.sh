...
nohup java -jar $(find . -maxdepth 1 -name demo-energy-forecast-indoor-provider-\*.jar | sort | tail -n1) >/dev/null 2>&1 &
echo indoor-provider started


cd ../../demo-energy-forecast-outdoor-provider/target
nohup java -jar $(find . -maxdepth 1 -name demo-energy-forecast-outdoor-provider-\*.jar | sort | tail -n1) >/dev/null 2>&1 &
echo outdoor-provider started

cd ../../demo-energy-forecast-provider/target
nohup java -jar $(find . -maxdepth 1 -name demo-energy-forecast-provider-\*.jar | sort | tail -n1) >/dev/null 2>&1 &
